...
- Wygląd: wybieramy jedną główną, bardzo ciemną identyfikację spin.clinic. Konto użytkownika docelowo oferuje trzy pełne szaty graficzne o wspólnej hierarchii i znaczeniu statusów. Znak czyta się ruchem zgodnym z zegarem: `S` góra, `P` prawa, `I` dół, `N` lewa. Rekomendowana typografia to IBM Plex Sans / Serif / Mono. Szczegóły: `docs/BRAND_AND_THEMES.md`.
