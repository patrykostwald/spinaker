...
- W MVP wklejenie publicznego URL wypowiedzi uruchamia dwa wyraźnie rozdzielone kroki. Najpierw płatny model przez adapter wyszukuje kontekst w lokalnej bazie, a kontrolowany research może dopisać kandydackie URL-e wyłącznie z dozwolonych źródeł. Osobny przycisk `Dr Spin` uruchamia ocenę twierdzeń według jawnych kategorii metodologicznych. Wynik jest analizą systemu z dowodami, zakresem pokrycia i niepewnością, a nie certyfikatem prawdy.
- Oficjalne płatne API X planujemy możliwie wcześnie po ustabilizowaniu ingestu. Pilot obejmuje bieżące posty ręcznie zweryfikowanej listy polityków i kont instytucjonalnych; klasyfikacja obozu i roli pochodzi z katalogu redakcyjnego. Pełne archiwum X ani użycie tych danych do treningu nie należą do MVP.
...
Wersjonowany przepływ od prywatnego oryginału przez ekstrakcję/OCR, segmenty, indeksy i kontrolę jakości opisuje `docs/DATA_ENRICHMENT_PIPELINE.md`. Ten kontrakt obowiązuje wyszukiwarkę, Dr Spina i przyszły własny silnik.
