...
type NewsCardProps = {
  ...
  onOpen?: (article: Article) => void; // портал; без него — обычная навигация <Link>
  onPreview?: (article: Article, originEl: HTMLElement) => void; // хэндофф ступени B слою R5
...
`PREVIEW_DELAY_MS`, или сразу по фокусу с клавиатуры)`. Если передан `onPreview`, карта **не**
переходит на локальное состояние `"b"` сама — она остаётся визуально на `"a"` и отдаёт намерение
слою-порталу (`useHoverExpand`, §3.9), который рисует растущий клон НАД страницей. Если
`onPreview` не передан, ступень `"b"` рисуется тут же, в потоке, абсолютным блоком
`.sc-card__expand` под нижней кромкой карты (см. §7, известные ограничения). Ступень C
(полный экран) компонент не рисует вообще — только вызывает `onOpen`.

`data-material-id={article.id}` на корневом `<article>` — по нему `PortalProvider` находит карту
(`document.querySelector('[data-material-id="…"]')`) и `useFocalBand` (мобильный слой) находит
кандидатов в фокальной полосе.
...
дочерний элемент. Рисует три вещи в одном слое: клон ступени B (`useHoverExpand()`), затемнение
(`.sc-portal-scrim`) и поверхность ступени C (`MaterialSurface`). Три случая возврата при `close()`
считаются **синхронно** внутри `PortalProvider.close()` (карта на экране / уехала из вида —
`scrollIntoView({behavior:"auto"})` в том же такте / размонтирована).

```ts
// useHoverExpand(): ReactNode  — клон ступени B, без своего <AnimatePresence>
...
`layoutId={sc-card-${id}}` — карточка, её клон-предпросмотр или оверлей; карточка снимает с себя
`layoutId`, когда его несёт клон/оверлей». **Фактическая реализация отличается** (задокументировано
прямо в коде, `PortalProvider.tsx`): `NewsCard.tsx` **никогда** не несёт `layoutId` вообще —
единственное дозволенное изменение в её файле — проп `onPreview`. Вместо этого:

- пока карта на ступени B, ровно **клон** в `PortalLayer` (`useHoverExpand`) несёт
  `layoutId={sc-card-${id}}`;
- если пользователь кликает по карте, у которой уже есть живой клон, `MaterialSurface` получает
  **тот же** `layoutId` в одном коммите с закрытием клона — framer-motion сам считает проекцию
  (translate + scale + коррекция радиуса) от текущего, «живого» бокса клона (`useSharedLayout=true`);
- если живого клона нет (клик до созревания 400ms, открытие без наведения, возврат по
  `?podglad=`/`history`), `NewsCard` нечего поймать — `MaterialSurface` анимирует явные
  `top/left/width/height/borderRadius` между `fromRect` (снятым `getBoundingClientRect()` в момент
  `open()`) и целевым боксом — ручной FLIP, не layoutId;
- закрытие в **обоих** случаях идёт назад к явному `exitRect`/`exitRadius`, посчитанным
  синхронно в `PortalProvider.close()` (три случая возврата), а не через `layoutId` — со стороны
  оригинала после закрытия снова нет ничего, что могло бы «поймать» оверлей.

Правило дословно выполняется только в двух других местах: `Dropdown` (семя в кнопке ↔ панель) и
мобильная панель `NavMenu` (семя в кнопке меню ↔ панель) — там общий `layoutId` действительно
переезжает между ровно одним живым элементом за раз, как в контракте.
...
- **Живые страницы — всё ещё IBM Plex.** `layout.tsx` подключает Montserrat только как переменную
  ...
  использоваться.
- **Временный in-card CSS ступени B.** `.sc-card[data-stage="b"] .sc-card__expand` (абсолютный блок
  под нижней кромкой карты, `clip-path` обрезающий ореол сверху) остаётся рендерером ступени B **для
  экземпляров `NewsCard`, которым не передан `onPreview`** — то есть для любой карты, смонтированной
  без `PortalProvider`/`PortalLayer` над ней. Как только `onPreview` подключён, карта отдаёт ступень
  B слою-порталу и свой `.sc-card__expand` не показывает.
...
1. **Правило владения `layoutId`** («Контракты» → NewsCard, «Портал» → «`layoutId` выдаётся только
   активной карточке») реализовано не буквально: `NewsCard.tsx` никогда не несёт `layoutId`. Реальный
   общий `layoutId="sc-card-${id}"` делят между собой только клон ступени B (`useHoverExpand`) и
   `MaterialSurface`; переход карта→клон→оверлей восстанавливается явным снятием
   `getBoundingClientRect()` в `open()`/`close()`, а не единой цепочкой `layoutId`. Отклонение
   задокументировано в самом коде (`PortalProvider.tsx`, `MaterialSurface.tsx`) как согласованное
   R5 в отчёте. Подробности — §4 выше.
...
3. **`NewsCardProps`** содержит `onPreview` (хэндофф ступени B слою R5) и `layout` (`"stack"|"split"`
   для `large`) — оба сверх набора, зафиксированного в «Контрактах» волны 0
