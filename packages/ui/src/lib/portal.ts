...
export type PortalConfig = { categories: CategoryOption[]; topics?: CategoryOption[]; sources?: Source[]; top_sources: Source[]; editorial: { government: ThreadDetail | null; opposition: ThreadDetail | null }; x_editorial: { configured: boolean; status: string } };
...
export function getNewsFeed({ mode = 'latest', query = '', categories = [], topics = [], sources = [], page = 1, pageSize = 20, match = 'substring' }: { mode?: string; query?: string; categories?: string[]; topics?: string[]; sources?: number[]; page?: number; pageSize?: number; match?: 'substring' | 'words' } = {}) {
