...
export function SignedOutPanel({ title = 'Twoje prywatne miejsce do pracy z materiałami' }: { title?: string }) {
  ...
  return (
    <section className="mvp-acc mvp-acc-signed-out" aria-labelledby="acc-signed-out">
      <p className="mvp-acc-kicker">MOJE KONTO</p>
...
function Section({ id, title, count, action, children }: { id: string; title: string; count?: number; action?: ReactNode; children: ReactNode }) {
  return (
    <section id={id} className="mvp-acc-section" aria-labelledby={`${id}-title`}>
      <header>
...
function QueryState({ query, unavailableText }: { query: { isPending: boolean; isError: boolean; error: unknown; refetch: () => unknown }; unavailableText: string }) {
  if (query.isPending) return <p role="status" className="mvp-acc-empty">Ładuję…</p>;
  if (query.isError && isUnavailable(query.error)) return <p className="mvp-acc-empty mvp-acc-unavailable">{unavailableText}</p>;
  if (query.isError) return <p role="alert" className="mvp-acc-empty">Nie udało się pobrać danych. <button type="button" className="quiet-button" onClick={() => query.refetch()}>Ponów</button></p>;
  return null;
}
...
function ThreadsSection() {
  ...
  return (
    <Section id="moje-nitki" title="Moje nitki kontekstowe" count={threads.isSuccess ? rows.length : undefined}
      action={threads.isError && isUnavailable(threads.error) ? null : <Link href="/konto/nitki/nowa" className="quiet-button mvp-acc-new">+ Nowa nitka</Link>}>
      <p className="mvp-acc-private"><span>PRYWATNE</span> Nitki widzisz tylko Ty. Nie są publikowane, nie są nitkami Dr Spina i nie układa ich AI.</p>
      ...
      {threads.isSuccess && !rows.length && (
        <div className="mvp-acc-empty">
          <p>Nie masz jeszcze żadnej nitki.</p>
          <p>Zacznij od tytułu, dodaj hasła i materiały z Bazy, a potem ustaw ich kolejność.</p>
        </div>
      )}
      {rows.length > 0 && (
        <ul className="mvp-acc-rows">
          {rows.map(thread => (
            <li key={thread.id}>
              <div>
                <Link href={`/konto/nitki/${thread.id}`} className="mvp-acc-title">{thread.title}</Link>
                {thread.description && <p className="mvp-acc-desc">{thread.description}</p>}
...
function ArticleFavoritesSection() {
  ...
  return (
    <Section id="ulubione-materialy" title="Ulubione materiały" count={favorites.isSuccess ? rows.length : undefined}>
      ...
      {favorites.isSuccess && !rows.length && <p className="mvp-acc-empty">Zapisz materiał znakiem ♡ w jego boxie albo w oknie materiału.</p>}
      ...
      {rows.length > 0 && (
        <ul className="mvp-acc-rows">
          {rows.map(row => (
            <li key={row.id}>
              <div>
                <p className="mvp-acc-meta"><span className="mvp-acc-tag">{categoryLabel(row.article.category)}</span> {row.article.published_date ? formatDateTimePl(row.article.published_date) : 'data publikacji nieznana'}</p>
                <Link href={`/material/${row.article.id}`} className="mvp-acc-title">{row.article.title}</Link>
...
function ThreadFavoritesSection() {
  ...
  return (
    <Section id="ulubione-nitki" title="Ulubione nitki Dr Spina" count={favorites.isSuccess ? rows.length : undefined}>
      ...
      {favorites.isSuccess && !rows.length && <p className="mvp-acc-empty">Zapisz opublikowaną nitkę redakcyjną przyciskiem ♡ Zapisz na jej stronie.</p>}
      {error && <p role="alert" className="mvp-acc-error">{error}</p>}
      {rows.length > 0 && (
        <ul className="mvp-acc-rows">
          {rows.map(row => (
            <li key={row.id}>
              <div>
                <p className="mvp-acc-meta"><span className="mvp-acc-tag">DR SPIN</span> zapisano <time dateTime={row.created_at}>{formatDateTimePl(row.created_at)}</time></p>
                <Link href={`/thread/${row.thread.slug}`} className="mvp-acc-title">{row.thread.title}</Link>
              </div>
      ...
      {favorites.data?.next_page && <p className="mvp-acc-more"><Link href="/profile">Wszystkie ulubione nitki w profilu →</Link></p>}
    </Section>
  );
}
...
function TopicsSection() {
  ...
  return (
    <Section id="paski" title="Moje paski tematów" count={topics.isSuccess ? rows.length : undefined}
      ...
      {topics.isSuccess && !rows.length && <p className="mvp-acc-empty">Pasek to zapisany widok Bazy: hasło, kategorie lub źródła. Możesz mieć do {topics.data.max_topics} pasków.</p>}
      {error && <p role="alert" className="mvp-acc-error">{error}</p>}
      {rows.length > 0 && (
        <ul className="mvp-acc-rows">
          {rows.map(topic => (
            <li key={topic.id}>
              <div>
                <p className="mvp-acc-title">{topic.label}</p>
                <p className="mvp-acc-meta">
...
function ActivitySection() {
  ...
  return (
    <Section id="aktywnosc" title="Ostatnia aktywność">
      <p className="mvp-acc-private"><span>PRYWATNE</span> Zbiorcza historia jest domyślnie prywatna. Komentarze pod materiałami są publiczne.</p>
      {loading && !items.length && <p role="status" className="mvp-acc-empty">Ładuję…</p>}
      {!loading && !items.length && <p className="mvp-acc-empty">Tu pojawią się Twoje reakcje, zapisane materiały i zmiany w nitkach.</p>}
      {items.length > 0 && (
        <ol className="mvp-acc-activity">
          {items.map(item => (
            <li key={item.key}>
              <time dateTime={item.at}>{formatDateTimePl(item.at)}</time>
              <span className="mvp-acc-tag">{item.kind}</span>
              <p>{item.text}</p>
            </li>
          ))}
        </ol>
      )}
      <p className="mvp-acc-more"><Link href="/profile">Pełna historia i ustawienia prywatności →</Link></p>
    </Section>
  );
}
  ...
  return (
    <div className="mvp-acc">
      <header className="mvp-acc-head">
        ...
        <nav className="mvp-acc-toc" aria-label="Sekcje konta">
