...
function OpinionContent({ article, ownerId }: { article: Article; ownerId: number | undefined }) {
  ...
  return <section className="article-opinions">
    <header className="strip-heading"><div><h2>Głosy czytelników</h2><p>Opinie o materiale · ocena należy do Ciebie</p></div></header>
    ...
    {opinions.data && <div className="opinion-columns">{(['negative', 'positive'] as const).map(side => {
      ...
      return <section key={side} className={`opinion-column opinion-${side}`} aria-label={`${side === 'negative' ? 'Negatywne' : 'Pozytywne'} opinie`}><h3>{side === 'negative' ? 'Negatywne' : 'Pozytywne'} <span>{opinions.data!.counts[side]}</span></h3>{rows.length ? rows.map(opinion => <article key={opinion.id} className="reader-opinion"><div className="flex flex-wrap items-baseline justify-between gap-2"><strong>@{opinion.author.username}</strong><time dateTime={opinion.created_at}>{formatDateTimePl(opinion.created_at)}</time></div><p>{opinion.body}</p><CopyPost text={opinion.body} articleId={article.id} /></article>) : <p className="opinion-empty">Nie ma jeszcze komentarzy w tej części.</p>}{nextPage && <button disabled={Boolean(loadingMore)} onClick={() => loadMore(side)} className="quiet-button">{loadingMore === side ? 'Ładuję…' : 'Kolejne opinie'}</button>}</section>;
    })}</div>}
    {error && <p role="alert" className="mt-3 text-sm text-red-700">{error}</p>}
    {!ownerId ? <div className="opinion-invitation"><p>Dodaj swój punkt widzenia.</p><button className="quiet-button" onClick={() => setAccountOpen(true)}>Zaloguj się, aby ocenić</button></div> : mine?.body ? <div className="opinion-confirmation"><p className="text-sm">Twoja opinia została dodana · {mine.polarity === 'positive' ? 'pozytywna' : 'negatywna'}.</p><CopyPost text={mine.body} articleId={article.id} /></div> : opinions.isSuccess ? <form onSubmit={publish} className="opinion-composer">
      {mine && <p role="status" className="mb-4 text-sm text-primary">Ocena zapisana · {mine.polarity === 'positive' ? 'pozytywna' : 'negatywna'}. Możesz jeszcze dopisać jeden komentarz.</p>}
      <fieldset disabled={Boolean(mine)}><legend className="mb-3 text-sm">Jak oceniasz ten materiał?</legend><div className="flex gap-3">{(['negative', 'positive'] as const).map(side => <label key={side} className={`opinion-choice opinion-${side}`}><input type="radio" name={`opinion-${article.id}`} value={side} checked={chosenPolarity === side} onChange={() => setPolarity(side)} required />{side === 'negative' ? 'Negatywnie' : 'Pozytywnie'}</label>)}</div></fieldset>
      <label className="mt-4 block text-sm">Twój komentarz {mine ? '' : '(opcjonalnie)'}<textarea value={body} onChange={e => setBody(e.target.value)} rows={3} maxLength={1000} className="mt-2 w-full rounded border bg-transparent p-3" placeholder="Odnieś się do materiału i jego kontekstu." /></label>
      <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500"><span className={stats.weightedLength > 240 || codepoints > 240 ? 'text-red-700' : ''}>{stats.weightedLength}/240 znaków ważonych · {codepoints}/240 znaków</span><span>Jedna ocena i opcjonalny komentarz. Opublikowanego komentarza nie można zmienić.</span></div>
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><button disabled={pending || !chosenPolarity || (body.trim() ? !bodyValid : Boolean(mine))} className="rounded bg-primary px-5 py-2.5 text-sm text-white disabled:opacity-40">{pending ? 'Zapisuję…' : mine ? 'Dodaj komentarz' : body.trim() ? 'Dodaj opinię' : 'Zapisz ocenę'}</button>{bodyValid && <CopyPost text={body.trim()} articleId={article.id} />}</div>
    </form> : null}
    <AccountDialog open={accountOpen} onClose={() => setAccountOpen(false)} />
  </section>;
}

export function ArticleOpinions({ article }: { article: Article }) {
