<#
Activates only the five public RSS channels reviewed on 2026-09-23.
...
$sources = @(
  ...
  @{ Id = 571; Terms = "https://bip.czestochowa.pl/artykul/71521/1151613/ponowne-wykorzystywanie"; Note = "Czestochowa BIP reuse conditions cover information published in its BIP. Metadata-only RSS import." },
...
foreach ($source in $sources) {
  docker compose exec backend python manage.py audit_sources --source-id $source.Id --force --workers 1
  docker compose exec backend python manage.py configure_audited_rss_metadata_source `
    ...
    --daily-cap 24 `
    --apply
  docker compose exec backend python manage.py fetch_approved_rss_source --source-id $source.Id
}
