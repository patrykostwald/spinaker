...
- MVP jest responsywnym portalem i instalowalną PWA ze wspólnego kodu. Natywne aplikacje i publikację w App Store/Google Play odkładamy poza MVP.
- Konto zwykłego użytkownika w MVP służy do personalizacji, przeglądania, ulubionych oraz jednej reakcji i jednego komentarza na box lub nitkę. Użytkownik nie tworzy ani nie publikuje własnych nitek. Autorów dopuszcza redakcja po zgłoszeniu.
- Własne paski użytkownika: co najmniej jeden pasek budowany z hasła, źródeł, kategorii lub ich połączenia, np. hasło ograniczone do wybranych źródeł albo sama kategoria `reportaże`. Są to zapisane widoki archiwum, a nie publikowane nitki autorskie.
- Kolejność głównej w MVP: `Wszystkie wiadomości` → `TOP 10` → `Top temat dnia` → jedna nitka `Dr Spin` (druga tylko wtedy, gdy dzień rzeczywiście tego wymaga) → jedna jawnie oznaczona nitka sponsorowana → nitki autoryzowanych dziennikarzy. Nie mnożymy stałych sekcji Dr Spin ani reklam.
- Redakcja zasila MVP gotowymi nitkami najważniejszych wydarzeń. W układzie nitki po lewej znajduje się zdarzenie lub punkt wyjścia, a po prawej przewijany chronologicznie pasek wszystkich powiązanych boxów. Kliknięcie rozwija pełny kontekst i publikacje pogrupowane według dat.
- Wygląd: domyślną identyfikacją jest ciemny **Monochrome Control**. Zarejestrowany użytkownik wybiera w profilu motyw Ciemny, Jasny lub Pastelowy; ustawienie synchronizuje się z kontem. Przełącznik w nagłówku ma postać trzech pasków w kolorach aktywnego motywu i oferuje również Auto zgodne z ustawieniem urządzenia. Marka używa samego wordmarku `spin.clinic`, bez osobnego sygnetu z liter. Rekomendowana typografia to IBM Plex Sans / Serif / Mono. Szczegóły: `docs/BRAND_AND_THEMES.md`.
...
- Płatne AI działa w MVP przez wymienny adapter i jest narzędziem redakcyjnym w tle, a nie publicznym czatem. Klasyfikuje materiały, wydobywa twierdzenia, proponuje powiązania, chronologię, temat dnia i szkice Dr Spina. Zapisujemy wejściowe identyfikatory, ustrukturyzowany wynik, dowody, dostawcę/model, wersję promptu, koszt, pewność, decyzję człowieka i korekty. To tworzy własny, audytowalny zbiór ewaluacyjny i przyszły treningowy; nie zakładamy przeniesienia pamięci ani wag usługowego modelu.
- Domyślny kontekst nie jest odpowiedzią generowaną przez AI. Po otwarciu boxa portal pokazuje wizualnie podstawowe, sprawdzalne relacje: chronologię, źródła pierwotne, wcześniejsze i późniejsze materiały, osoby, organizacje, cytowane dokumenty oraz znane korekty. Użytkownik sam odczytuje obraz z dowodów. Aktywna analiza jest osobnym, świadomie uruchamianym działaniem dla konkretnego URL lub boxa.
- Robocza nazwa płatnej analizy na żądanie to `Dr Spin`. Użytkownik wkleja URL albo uruchamia analizę z boxa; system identyfikuje materiał, szuka jego kontekstu w bazie, rozbija sprawdzalne twierdzenia, pokazuje dowody i braki oraz wyjaśnia wynik jak narzędzie OSINT. Nie każde badanie musi zakończyć się etykietą „spin”. Wynik odróżnia fakt, relację danych, wniosek modelu i niepewność.
...
- Dr Spin na X: w MVP AI przygotowuje dowody i szkice, a wynik publikuje instytucjonalnie `Redakcja spin.clinic`; później kontrolowana autopublikacja obejmie tylko jednoznaczne wyniki niskiego ryzyka, z audytem i możliwością natychmiastowego wycofania. Szczegóły: `docs/X_EDITORIAL_PIPELINE.md`.
- X w MVP: oficjalne płatne API, ręcznie zweryfikowany pilot 100–150 najważniejszych kont, bieżące posty zamiast pełnego archiwum. Obóz/rola pochodzą z jawnych danych redakcyjnych, nie z inferencji modelu. AI przygotowuje kartę twierdzeń i powiązane boxy, ale publiczna etykieta, nitka i post wymagają zatwierdzenia redaktora. Brak automatycznych odpowiedzi politykom. Szczegóły: `docs/X_EDITORIAL_PIPELINE.md`.
- Udostępnianie na X w MVP: przycisk otwiera przygotowany formularz publikacji z tekstem i publicznym linkiem do boxa lub nitki w spin.clinic. Link pokazuje kontekst oraz odsyła do oryginału. Ta funkcja nie wymaga automatycznego publikowania przez konto użytkownika. Publikowanie z konta spin.clinic pozostaje ręczną decyzją redakcji.
...
- Produkt zawodowy dla dziennikarzy pozostaje przygotowany architektonicznie, lecz nie rozszerza zakresu premiery MVP. Korzysta z jednego kanonicznego magazynu materiałów przez osobny, ograniczony widok: boxy, indeks źródeł, chronologia, relacje, alerty i eksport bez komentarzy, reakcji, społeczności oraz Dr Spina. Nie utrzymujemy dwóch rozjeżdżających się baz. Szczegóły: `docs/CONTEXT_ARCHIVE_PRODUCT.md`.
- Model dostępu rozwijamy warstwowo: podstawowy kontekst wizualny pozostaje szeroko dostępny, natomiast kosztowne analizy URL, głębsze uzasadnienia, większe limity, alerty zawodowe i eksporty mogą należeć do płatnych kont. Granica płatności nie może ukrywać pochodzenia ani dowodów potrzebnych do oceny publicznie pokazanej etykiety.
...
- Harvestery skalujemy maksymalnie w granicach polityki każdego źródła i zmierzonej wydajności. Kolejne komputery/VPS mogą pracować jako rozproszone workery na wspólnej kolejce, z dzierżawą zadania, idempotentnym zapisem i globalnym limitem na domenę. Dodatkowych adresów IP nie używamy do obchodzenia 429, robots.txt ani blokad; sama liczba IP nie może zwiększać dozwolonego obciążenia wydawcy.
...
6. Podłączyć publiczne boxy, klasyczną wyszukiwarkę archiwum, personalizację, reakcje i komentarz oraz zgłoszenie błędu. Kliknięcie boxa pokazuje kontekst z bazy. Redakcyjne nitki i szkice AI publikowane po zatwierdzeniu redakcji; publiczna rozmowa z AI nie jest warunkiem MVP.
...
9. AI-01 — OpenAI i Mistral przez wspólny adapter dla redakcji; wybór do 15 powiązanych materiałów z realnej bazy. Odbiór: wyłącznie istniejące i dozwolone ID boxów, daty i pochodzenie, brak fikcyjnych linków, granica kosztu/liczby prób oraz zatwierdzanie przez redakcję. Zestaw ewaluacyjny powstaje z zaimportowanych rekordów, gdy baza osiągnie użyteczne pokrycie.
