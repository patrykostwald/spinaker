...
export default function ThreadPage({ params }: { params: { slug: string } }) {
  ...
  return <article className="sc-thread-page"><Button href="/" variant="quiet" size="sm">Wszystkie historie</Button>
    ...
    {thread.author_name && <p className="sc-t-meta thread-note-author" data-author-role={thread.author_role}>{thread.author_name} · {thread.author_role === 'journalist' ? 'Dziennikarz' : thread.author_role === 'editor' ? 'Redakcja' : 'Autor'}</p>}
