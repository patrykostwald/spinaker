...
from scraper.backfill import (PILOT_CUTOFF_AT, approved_source_ids, parse_cutoff,
                              run_backfill)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--dynamic-sources', action='store_true',
            help='Re-read the verified local source allowlist before every cycle.')
    ...
    def handle(self, *args, **options):
        ...
        dynamic = options['dynamic_sources']
        if not 1 <= workers <= 32:
        ...
        if not dynamic and not options['source_id']:
            raise CommandError('source-id is required unless --dynamic-sources is used')
        cutoff = parse_cutoff(options['cutoff_at'])
        static_source_ids = list(dict.fromkeys(options['source_id'] or []))
        deadline = time.monotonic() + max_hours * 3600
        cycles = completed = 0
        while time.monotonic() < deadline:
            source_ids = approved_source_ids() if dynamic else static_source_ids
            ...
            if not dynamic and not pending_now and not future_retry:
