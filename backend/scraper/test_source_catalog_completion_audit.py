...
@pytest.mark.django_db
def test_completion_audit_accepts_a_valid_html_access_card(tmp_path):
    active = Source.objects.create(name='HTML active', url='https://active.example',
        source_type=SourceType.INSTITUTION, catalog_stage='configured', is_active=True, scrape_enabled=True)
    SourceAccessInstruction.objects.create(source=active, version=1, status='approved', channel='html',
        allowed_scope='metadata', endpoint='https://active.example/aktualnosci', terms_url='https://active.example/terms',
        evidence={'review_note': 'test'}, reviewed_at=timezone.now(), reviewed_by='test',
        valid_until=timezone.now() + timedelta(days=1), daily_request_cap=24)
    stream = StringIO()
    call_command('source_catalog_completion_audit', '--output', str(tmp_path / 'audit.md'), '--strict', stdout=stream)
    assert 'active_cards=1/1' in stream.getvalue()
    assert 'complete=true' in stream.getvalue()
