...
class MistralEUSelectionProvider:
    """EU-regional chat completion returning only supplied candidate IDs."""
        ...
        payload = {
            ...
            'messages': [
                ...
                {'role': 'user', 'content': json.dumps(
                    {'topic': topic, 'candidates': candidates}, ensure_ascii=False)},
            ],
            'temperature': 0,
            'max_tokens': 1200,
            'response_format': {'type': 'json_schema', 'json_schema': {
                'name': 'context_selection', 'schema': schema, 'strict': True}},
        }
        deadline = time.monotonic() + 60
        try:
            with requests.post(
